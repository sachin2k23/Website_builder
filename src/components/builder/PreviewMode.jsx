...
function getElementHref(el) {
  if (el.linkType === 'url') return el.linkHref || el.link || ''
  if (el.linkType === 'page') {
    if (!el.linkPage) return ''
    return `/${el.linkPage}${el.linkPageAnchor ? `#${el.linkPageAnchor}` : ''}`
  }
  if (el.linkType === 'anchor') return el.linkAnchor ? `#${el.linkAnchor}` : ''
  return el.link || ''
}

function LinkOverlay({ el, layout, onNavigatePage }) {
  const href = getElementHref(el)
  if (!href || el.linkType === 'none') return null

  const handleClick = (event) => {
    if (el.linkType === 'page') {
      event.preventDefault()
      onNavigatePage?.(el.linkPage, el.linkPageAnchor)
      return
    }
    if (el.linkType === 'anchor') {
      event.preventDefault()
      document.getElementById(el.linkAnchor)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }
  }
  ...
  return (
    <a
      ...
      rel={(el.linkTarget || '_self') === '_blank' ? 'noopener noreferrer' : undefined}
      onClick={handleClick}
      aria-label={el.content || el.label || `Open ${href}`}
      style={{
        position: 'absolute',
        left: layout.x || 0,
        top: layout.y || 0,
...
function renderElement(el, activeBreakpoint, onNavigatePage) {
  ...
  switch (el.type) {
    ...
    case 'image':
      node = (
        <div style={{ ...base, width: `${w}px`, height: `${h}px`, backgroundColor: el.fill || '#F3F6FB', borderRadius: `${radius}px`, border: border || '1.5px dashed #D8E1F0', boxShadow: shadow, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {el.src
    ...
    case 'video':
      node = (
        <div style={{ ...base, width: `${w}px`, height: `${h}px`, backgroundColor: el.fill || '#0F1A2E', borderRadius: `${radius}px`, border, boxShadow: shadow, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    case 'checkbox':
      ...
      break
    case 'icon':
      node = <div style={{ ...base, width: `${w}px`, height: `${h}px`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: el.textColor || '#2348D7', fontSize: Math.min(w, h) * 0.65 }}>★</div>
      break
    default:
      return null
  }
  ...
  return (
    <div key={el.id}>
      {node}
      <LinkOverlay el={el} layout={layout} onNavigatePage={onNavigatePage} />
    </div>
  )
}

export default function PreviewMode({ pages = [], treeByPage = {}, activePageId = 'home', canvasSettings, onExit }) {
  ...
  const navigatePage = (pageId, anchor) => {
    if (!treeByPage[pageId]) return
          ...
          <>
            <div style={{ width: scaledDisplayW, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, paddingLeft: 4, paddingRight: 4, flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 12, color: '#5E6F8E', fontWeight: 500, fontFamily: 'Inter, sans-serif' }}>{previewPage?.name || current.label}</span>
